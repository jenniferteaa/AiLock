# This code is implemented to identify the best ai.lock parameters which will then be used to the performance of ai.lock under the attack datasets.
# We identify the best parameters for the ai.lock variants using 5 fold cross validation on the ai.lock training dataset.
# pip install nearpy
# Transform_PCA and Test_case_attack_creator are .py files that need to be imported.
...
sys.path.append("/usr/local/lib/python3.10/dist-packages")  # the path to nearpy lib
sys.path.append("/content/MyDrive/MyDrive/Colab Notebooks")
...
def split_data_to_test_train(dataset, num_folds, index_train_fold):
    ...
    """
    fold_size = int(dataset.shape[0] / num_folds)
...
# LSH is used to calculate the imageprints, that is, LSH maps the extracted features from the .h5 file and maps
# It to the respective binary format which we call the imageprint
...
def data_for_experiment(basic_path, layer_names, num_folds, experiment, projection_count, start_pc_component, end_pc_component):
    ...
    pc_test_nexus, pc_test_aloi, pc_test_google = data_for_layer(basic_path, num_folds, experiment, projection_count, start_pc_component, end_pc_component)
    for layer in range(1, len(layer_names)):
        print("layer {}".format(layer_names[layer]))
        pc_test_nexus1, pc_test_aloi1, pc_test_google1 = data_for_layer(basic_path, layer_names[layer], num_folds, experiment, projection_count, start_pc_component, end_pc_component)
...
def data_for_layer(basic_path, num_folds, experiment, projection_count, start_pc_component, end_pc_component):
    # Read datasets
    basic_path_layer = os.path.join(basic_path, "Mixed10")

    dataset_files = "/content/MyDrive/MyDrive/Mixed10/FullSize/TOYS_train_111.h5"
    hd = h5py.File(os.path.join(basic_path_layer, "full_size", dataset_files), 'r')
    dataset_toys = hd['dataset_1']
    dataset_train_toys, dataset_test_toys = split_data_to_test_train(dataset_toys, num_folds, experiment)
    del dataset_toys
    transformer = TransformImagesPCA(n_components=89)
    transformer.learn_pcs(dataset_train_toys)
    del dataset_train_toys

    dataset_files = "/content/MyDrive/MyDrive/Mixed10/FullSize/Google_train_429.h5"
    hd = h5py.File(os.path.join(basic_path_layer, "full_size", dataset_files), 'r')
    ...
    dataset_files = "/content/MyDrive/MyDrive/Mixed10/FullSize/NexusFull_train_215.h5"
    hd = h5py.File(os.path.join(basic_path_layer, "full_size", dataset_files), 'r')
    ...
    pc_test_toys = transformer.transform(dataset_test_toys)[:, start_pc_component:end_pc_component]
    ...
    pc_test_toys = project_LSH(pc_test_toys, rbp)
    pc_test_google = project_LSH(pc_test_google, rbp)
    return pc_test_nexus, pc_test_toys, pc_test_google
...
def perform_test(pc_nexus, pc_toys, pc_google):
    # Train Threshold
    labels_nexus = []
    ...
    # Toy Training
    scores_list, labels_train = find_scores_attack(pc_nexus, pc_toys)
...
#The accuracy and the F1 score for the test pair labels and the scores is calculated using the following.
...
def train(basic_path, layer_names, out_path, num_folds, experiment, projection_count, start_pc_component, end_pc_component, thresholds):
    """
    Finds the best performing threshold for binary classification
    """
    pc_test_nexus, pc_test_aloi, pc_test_google = \
        data_for_experiment(basic_path, layer_names, num_folds, experiment, projection_count, start_pc_component, end_pc_component)
    # on  Test
    overall_target, overall_score = perform_test(pc_test_nexus, pc_test_aloi, pc_test_google)
    tpr, fpr, fnr, tnr, accuracy, f_measure = apply_threshold_fast(thresholds, overall_target, overall_score, pos_label=1.0)
    ...
    with open(os.path.join(out_path, "fold_test_performance.txt"), 'a') as file:
        ress = [thresholds[arg_max], tpr[arg_max], fpr[arg_max], fnr[arg_max], tnr[arg_max], accuracy[arg_max], f_measure[arg_max]]
...
#This part can be used to produce attack paris, that could be used to test the application. 
#Replace the start and the end parameters according to the Nexus dataset image numbers.
...
def main():
    ...
    for pc_index in range(len(start_index_list)):
        ...
        for n_proj in num_projections:
            ...
            out_path = "multilayer" + \"/components = " + str(start_index) + "-" + str(start_index + n_components) + "/n_proj=" + str(
                n_proj)
            mkdir_p(out_path)
            for experiment in range(0, num_folds):
                print("fold: {}".format(experiment))
                best_threshold, perf = train(basic_path, layer_names, out_path, num_folds, experiment, n_proj,start_index, start_index + n_components, thresholds)
            ...
            with open(os.path.join(os.path.dirname(out_path), "best_average_performance_test.txt"), "a") as file:
                ...
                record = str(start_index) + "-" + str(start_index + n_components) + "\t" + str(n_proj) + "\t" + str(thresholds[arg_max]) + "\t" \+ recfile.write(record + "\n")
